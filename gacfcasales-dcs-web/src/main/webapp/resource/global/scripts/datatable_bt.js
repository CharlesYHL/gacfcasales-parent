...
var Datatable = function() {
    ...
    var defaultOperateFormat={
    		...
    		download:{template:'<a class="btn" href="javascript:;" title="{0}" data-url="{1}" data-model="{2}"  data-toggle="confirmationDown" data-method="{3}" {5} {6} {[disabled]}>{4}</a>',operateIcon:'<i class="fa fa-plus-square"></i>',defaultTitle:"下载",defaultMethod:"downLoad"}
    };
...
//    		 table.bootstrapTable('resetView');
    ...
    function extendAutoValueFormat(item,optionsExtend){
    	if(item.autoValueFormat){
    	 ...
    	 item.align=item.align || "right"; //设置默认对齐
   ...
   function returnAutoValueFormat(item,value, row, index){
    ...
    function extendCodeFormat(item,optionsExtend){
		 if(item.codeFormat){
			 ...
			 item.formatter = function(value, row, index){
				 ...
				 var result =  null;
				 if(type=='dict'){
					 result = dmsDict.getCodeDescById(type,codeType,value);
		 ...
		 if(item.operateFormat){
			 ...
			 $.each(item.operateFormat,function(j,item){
				 ...
				 if(item.type&&(item.type=="edit"||item.type=="detail")){
					 beforeEditRowEvent(item);
					 formats.push(handelEditADetail(templete,item,defaultSetting,operateEvents));
				 ...
				 }else if(item.type&&(item.type=="download")){
					 //后台删除
					 formats.push(handelDownLoad(templete,item,defaultSetting,optionsExtend));
		...
		item.onclick = function(value, row, index,obj) {
			...
			if(typeof beforeResult == "object"){
				...
				for(var i = 0; i< tableOptions.columns.length; i ++){
                    if(tableOptions.columns[i].footerFormatter){
                    	var temp = [];
                    	temp.field = tableOptions.columns[i].field;
                    	temp.index = i;
                        footCell.push(temp);
                    }
                }
               for(var i = 0; i < footCell.length; i++){
                    var amount = 0.00;
                    for(var j = 0; j < tableDatas.length; j++){
                    	if(tableDatas[j].statusitem != "D"){
                            amount += Number(checkNumber(tableDatas[j][footCell[i].field]));
                    	}
                    }
                    $(".fixed-table-footer tbody td:eq("+footCell[i].index+") .th-inner").text(amount.toFixed(2));
                }
     ...
     * 返回格式化字符串,针对页面
     */
    function handelEditADetail(templete,item,defaultSetting,operateEvents){
    	var args = [];
    	 var url = "";
		 //定义跳转页面
     ...
     * 返回格式化字符串,针对页面
     */
    function handelDownLoad(templete,item,defaultSetting,optionsExtend){
    	var args = [];
		 //定义跳转页面
    	 var title;
    	 if(item.title){
     ...
     * 编辑前事件
     */
    function beforeEditRowEvent(item) {
    	item.onclick = function(value, row, index,obj) {
    	    var beforeResult;
 			if (item.onBeforeEvent) {
 				beforeResult = executeRowEvent(value, row, index,item.onBeforeEvent,obj);;
     ...
     * 刷新合计列
     */
    function fushFootData(delSign,delSignVal){

		var tableDatas = table.bootstrapTable("getData");
		var footCell = [];
		var delSignType = false;
		if(delSign && delSignVal){
			delSignType = true;
		}
       ...
       for(var i = 0; i < footCell.length; i++){
            ...
            for(var j = 0; j < tableDatas.length; j++){
            	if(delSignType){
            		if(tableDatas[j][delSign] != delSignVal){
            			amount += Number(checkNumber(tableDatas[j][footCell[i].field]));
            		}
            	}else{
            		amount += Number(checkNumber(tableDatas[j][footCell[i].field]));
  ...
  //添加合计
    ...
    return {
        ...
        fushFootData:function(delSign,delSignVal){
        	fushFootData(delSign,delSignVal);
        }
    };
};