...
function formatDictSpanDate(obj,objValue){
	var dictCode = $(obj).attr("data-dictCode");
	
	return dmsDict.getCodeDescById('dict',dictCode,objValue);
}
...
function dmsFormToJson(container,serializeObj,filterFunction){
	...
	var inputArray = $('input[type="text"],input[type="hidden"],input[type="password"],textarea,input[type="radio"][data-isRadio="true"]:checked,div.radio-list input[type="radio"]:first-child,td.bs-checkbox > input[type="radio"],div.checkbox-list input[type="checkbox"]:first-child,td.bs-checkbox > input[type="checkbox"],select,input[data-role="tagsinput"]',container);
	if(filterFunction){
	...
	inputArray.each(function(index,item) {
		...
		var value = dmsCommon.getDmsValue(item,container);
		if(value){
...
var dmsCommon = function() {
	...
	var DMS_PATH={
		...
		fileUpload:{
			defaultUrl:{
				upload:"/dms.web/web/rest/basedata/upload",
				review:"/dms.web/web/rest/basedata/download/billFiles/",
				download:"/dms.web/web/rest/basedata/file/billFilesDownload/",
				del:"/dms.web/web/rest/basedata/file/delete/",
				detailReview:"/dms.web/web/rest/basedata/file/billFilesReview/"
			},
			dcs:{
				upload:"/dcsfactorybase/factoryBase/filedata/upload",
				review:"/dms.basedata/dmsWebRest/file/billFiles/",
				download:"/dms.basedata/dmsWebRest/file/billFilesDownload/",
				del:"/dms.basedata/dmsWebRest/file/delete/",
				detailReview:"/dms.basedata/dmsWebRest/file/billFilesReview/"
			},
			dcstech:{
				upload:"/dcsfactorybase/factoryBase/filedata/uploadtech",
				review:"/dms.basedata/dmsWebRest/file/billFiles/",
				download:"/dms.basedata/dmsWebRest/file/billFilesDownload/",
				del:"/dms.basedata/dmsWebRest/file/delete/",
				detailReview:"/dms.basedata/dmsWebRest/file/billFilesReview/"
	...
	/*	dmsCommon.ajaxRestRequest({
			url:dmsCommon.getDmsPath()["manage_root"]+"/users/info",
			type:'GET',
			async:false,
	        sucessCallBack:function(data){
	        	commonDataMap = $.extend(commonDataMap,data);
	        }
		});*/
	...
	function getDmsFuncIdUrl(url,urlToken){
		if(!isStringNull(url)){
			var newUrl = url;
			var dmsFuncId = $("#dmsPageContent").size()>0?$("#dmsPageContent").data("dmsFuncId"):dmsCommon.getDmsActiveFuncTab().data("dmsFuncId");
			if(dmsFuncId){
	...
	var handleDatePickers =  function(container) {
		if (jQuery().datepicker) {
			...
			//日期范围，日期控件2
			$('div.input-daterange>input',container).each(function(index,item){
				//默认配置
				var defaultOption = {
					rtl : App.isRTL(),
					orientation : "auto",
				...
				var datepickerEl = $(this).datepicker(options);
				//如果设置了默认当天
				var $item=$(this).parents('.input-daterange:first');
				var defaultToday = $item.attr("data-defaultToday");
				if(defaultToday&&defaultToday=="true"){
	        	...
	        	if($item.attr("data-defaultCurrMonth")=="true"){
	        		var index = $("input[type='text']",$item).index(this);
	        		if(index==0){
	        			var currVal = $(this).val();
						var currDate = moment().startOf('month').format("YYYY-MM-DD");
						if(isStringNull(currVal)){
							$(this).val(currDate);
						}
	        		}
                    if(index==1){
                    	var currVal = $(this).val();
    					var currDate = moment().endOf('month').format("YYYY-MM-DD");
    					if(isStringNull(currVal)){
    						$(this).val(currDate);
    					}
	        		}
	        	...
	        	if($item.attr("data-defaultDays")){
	    			...
	    			if(index==0){
	        			var currVal = $(this).val();
						var currDate = moment().subtract(parseInt($item.attr("data-defaultDays"))-1,'days').format("YYYY-MM-DD");
						if(isStringNull(currVal)){
							$(this).val(currDate);
						}
	        		}
	        	...
	        	if($item.attr("data-defaultFutureDays")){
                    ...
                    if(index==1){
                    	var currVal = $(this).val();
    					var currDate =moment().add(parseInt($item.attr("data-defaultFutureDays"))-1,'days').format("YYYY-MM-DD");
    					if(isStringNull(currVal)){
    						$(this).val(currDate);
    					}
	        		}
	        	...
	        	//定义默认月初到当前日期
	        	if($item.attr("data-defaultCurrDay")=="true"){
	    			var index = $("input[type='text']",$item).index(this);
	    			if(index==0){
	        			var currVal = $(this).val();
						var currDate = moment().startOf('month').format("YYYY-MM-DD");
                    ...
                    if(index==1){
                    	var currVal = $(this).val();
    					var currDate =moment().subtract(0,'days').format("YYYY-MM-DD");
    					if(isStringNull(currVal)){
    						$(this).val(currDate);
    					}
	        		}
	        	...
	        	//定义默认从本年1月1号到今天
	        	if($item.attr("data-defaultYearToNow")=="true"){
	    			var index = $("input[type='text']",$item).index(this);
	    			if(index==0){
	        			var currVal = $(this).val();
						var currDate = moment().startOf('year').format("YYYY-MM-DD");
						if(isStringNull(currVal)){
							$(this).val(currDate);
						}
	        		}
			...
			//日期范围，日期控件
			$('div.input-date-picker-range>input',container).each(function(index,item){
				//默认配置
				var defaultOption = {
					rtl : App.isRTL(),
					orientation : "auto",
				...
				var datepickerEl = $(this).datepicker(options);
				//如果设置了默认当天
				var $item=$(this).parents('.input-date-picker-range:first');
				var defaultToday = $($item).attr("data-defaultToday");
				if(defaultToday&&defaultToday=="true"){
					var currVal = $(this).val();
					var currDate = moment().format("YYYY-MM-DD");
				...
				//定义默认当月
	        	if($item.attr("data-defaultCurrMonth")=="true"){
	        		if($("input:first",$item).attr("id")==$(item).attr("id")){
	        			var currVal = $(this).val();
						var currDate = moment().startOf('month').format("YYYY-MM-DD");
						if(isStringNull(currVal)){
							$(this).val(currDate);
                    ...
                    if($("input:last",$item).attr("id")==$(item).attr("id")){
                    	var currVal = $(this).val();
    					var currDate = moment().endOf('month').format("YYYY-MM-DD");
    					if(isStringNull(currVal)){
    						$(this).val(currDate);
				...
				$(this).on("changeDate",function(event){
					event.preventDefault();
					//执行focus 功能
					focusElement($("input:first",$item));
					$(this).datepicker("hide");
					$("input:first",$item).trigger("changeDate.dms");
				});
	...
	var handleDateRangePickers = function (container) {
        ...
        $('div.input-daterange_discharge',container).each(function(index,item){
        	var defionOptions = {};
        	...
        	if($(item).attr("data-defaultCurrDay")=="true"){
        		defionOptions.startDate = moment().startOf('month');
    			defionOptions.endDate = moment().subtract(0,'days');
        	}
        	
        	//定义默认从本年1月1号到今天
        	if($(item).attr("data-defaultYearToNow")=="true"){
        		defionOptions.startDate = moment().startOf('year');
    			defionOptions.endDate = moment();
        	}
...
var futureDateRangePickers = function (container) {
	...
	var handleDatetimePicker = function (container) {
        ...
        $("div.datetime",container).each(function(index,item){
        	...
        	}else if($(dateTimeObj).attr("data-dateEndDate")=="endTimeSupposed"){
        		defionOptions.endDate = moment().subtract(-parseInt(dmsCommon.getSystemParamInfo("1063", "1063")),'minute').endOf('minute').format("YYYY-MM-DD HH:mm");
        	}else if($(dateTimeObj).attr("data-dateBeginDate")=="now"){
...
//        $('body').removeClass("modal-open"); // fix bug when inline picker is used in modal
    ...
    var handleBootstrapConfirmation = function(container) {
        ...
        //表格附件下载
        $.each($('[data-toggle=confirmationDown]',container),function(i,confirmationObj){
        	//获得按钮名称
        	var btnText = getBtnName(confirmationObj);
        	//创建confirm 事件
        	confirmElement(confirmationObj,"是否确认下载"+btnText+"?",true,function(confirmObj){
        		if(($(confirmObj).attr("data-fileUploadBtn")=="true"||$(confirmObj).attr("data-method")=="importData")&&$("input[type=file]",container).size()>0){
        			$(confirmObj).trigger("dms.upload");
        		}else{
        			ajaxRest(confirmObj,container);
...
//    		$(divContinaer).trigger("change");
	...
	/**
	 * 进行数据绑定，记录change的上一个值
	 */
	function changeFunc(e,self_obj,callBack){
		var changeObj = $(self_obj);
		var oldValue = $(changeObj).data("lastValue");
		var newValue = $(changeObj).getDmsValue();
		if(oldValue == undefined){
			$(changeObj).data("lastValue",newValue==undefined?"":newValue);
		}
		var returnFlag = callBack(changeObj,e);
		if(returnFlag == undefined||returnFlag == true){
			$(changeObj).data("lastValue",newValue==undefined?"":newValue);
		}
	}
...
//        		});
		...
		$('input[data-valueUrl],select[data-valueUrl],textarea[data-valueUrl],span[data-valueUrl]',container).each(function(index,obj) {
			...
			if(dataValueUrl){
				...
				if("parameter"==dataValueType){
					dataValueModel = "manage";//dms_basedata
				}else{
	...
	var handelFileDataUpload = function(container){
		...
		$('a[data-method="importData"]',container).each(function(index,obj){
			...
			//文件上传初始化
			$('input[type="file"].importData',container).each(function(i,fileImportObj){
				//扩展自定义属性
				var defineFileOption = {};
				if($(fileImportObj).attr("data-fileExtensions")){
					defineFileOption.allowedFileExtensions = $(fileImportObj).attr("data-fileExtensions").split(",");
				}
				//进行属性扩展
				var fileUploadObjOption = $.extend(true,fileUploadObjDefaultOption,defineFileOption);
				//构造附件导入功能
				$(fileImportObj).fileinput(fileUploadObjOption)
	...
	var handelFileAttacheUpload = function(container){
		...
		if(importFiles&&$(importFiles).size()>0){
			...
			importFiles.each(function(fileindex,fileInput){
				...
				//如果没有指定url，则使用默认的URL
				var fileUrlObj = getFileUploadUrlObj(fileInput);
				
				//如果不是在表格里的fileInput
				//var isTable = $(fileInput).closest("table").size()>0;
				var isTable = $(fileInput).closest("table").size()>0||$(fileInput).attr("data-table");
				if(!isTable){
					if(fileBillid&&fileBillType){
						//进行ajax 请求
						ajaxRestRequest({
							url:getDmsFuncIdUrl(fileUrlObj.review+fileBillType+"/"+fileBillid),
							type:"GET",
				            sucessCallBack:function(response){
				            	var initialPreview = response.initialPreview;
				            	//改变URL 的值
				            	$.each(response.initialPreview,function(previewIndex,priviewUrl){
				            		response.initialPreview[previewIndex] = getDmsFuncIdUrl(fileUrlObj.detailReview+priviewUrl,currentToken);
				            	});
				            	$.each(response.initialPreviewConfig,function(previewIndex,priviewConifg){
				            		...
				            		previewConfig.url = getDmsFuncIdUrl(fileUrlObj.del+previewConfig.url,currentToken);
				            	});
	 ...
	 * 获得文件上传相关的信息
	 */
	function getFileUploadUrlObj(fileInput){
		var fileAppUrl = $(fileInput).attr("data-fileAppUrl");
		var fileUrlObj;
		if(!fileAppUrl){
			fileUrlObj = dmsCommon.getDmsPath()["fileUpload"].defaultUrl;
		}else{
			fileUrlObj = dmsCommon.getDmsPath()["fileUpload"][fileAppUrl];
		}
		return fileUrlObj;
	}
	...
	var initFileInputWithoutPriview = function(fileInput,defineOption,container){
		...
		var $hidden = $(fileParentContainer).find('input[type=hidden][name="' + $(fileInput).attr("data-inputName") + '"]');
	    if ($hidden.length === 0) {
	        $hidden = $('<input type="hidden" name="'+$(fileInput).attr("data-inputName")+'">').insertBefore($(fileInput))
	    }
		//如果是上次已经上传的文件
		...
		//定义默认属性
		var fileUploadObjDefaultOption = $.extend(true,{},{
			 uploadUrl: getDmsFuncIdUrl(fileUrlObj.upload),
			 language:"zh",
			 showClose:false,
			 showPreview:false,
			 showUpload:false,
		...
		.on("filecleared",function(e,previewId, i){
		...
		}).off("fileuploaderror").on("fileuploaderror",function(e,outData, msg){
			if(msg){
				handelBootstrapToastr({status:"error",msg:msg,timeOut:"10000"});
				return;
			}
			//执行处理逻辑
			var fileInputObj = this;
			...
			if(statusCodeHandelObj[statusCode]){
				...
				//报错
				handelBootstrapToastr({status:"error",msg:"</br>原因：文件过大、格式不正确、请求超时，请删除重试",timeOut:"10000"});
			}
		}).off("fileloaded").on("fileloaded",function(e,file, previewId, i, reader){
			var fileInputObj = this;
			$(fileIdInputElement).val("");
			$(fileInputObj).removeAttr("data-isComplete");
		}).off("fileuploaded").on("fileuploaded",function(e,outData,thumbId,i){
			var fileInputObj = this;
			...
			if(key){
				$(fileCaptionDiv).on("click",function(event){
					if($(fileCaptionDiv).find("i").size()>0){
						//执行下载
						downLoadWithoutForm(fileCaptionDiv,fileUrlObj.download+billType+"/"+key);
					}
				});
			}
		});
	...
	function isCanPostRequest(container){
		...
		var formContaner = $(container).closest(DMS_CLOSEST_DIV).size()>0?$(container).closest(DMS_CLOSEST_DIV):container;
...
//			});
				...
				if(fileInputDataObj){
					if(fileInputDataObj.getFilesCount()>0){
						...
						if(fileCount>=$(fileInput).data('fileinput').minFileCount&&fileCount<=$(fileInput).data('fileinput').maxFileCount){
							...
							handelBootstrapToastr({status:"error",msg:"</br>最少上传"+$(fileInput).data('fileinput').minFileCount+"张附件",timeOut:"10000"});
						}
					}
				}
			}
	...
	function downLoadRequest(option){
		...
		//增加dmsFuncId
		var dmsFuncId = $("#dmsPageContent").size()?$("#dmsPageContent").data("dmsFuncId"):dmsCommon.getDmsActiveFuncTab().data("dmsFuncId");
	...
	var requestRestFormObj = function(obj,container){
		...
		if(url){
			...
			var isRest = $(obj).attr("data-isRest")=="false" ?false:true;
	...
	var ajaxRestRequest = function(option) {
		...
		$.ajax({
			...
			beforeSend:function(xhr) {
                ...
                xhr.setRequestHeader("Authorization", "Bearer "+getParamsStorage('Token'));
	...
	function fileUploadStatusCodeHandel(option,isRest){
		return {
			...
			403 : function(response, stausText, xhr) {
				handelBootstrapToastr({status:"error",msg:"</br>没有操作此功能的权限",timeOut:"10000"});
				return;
			},
			...
			"timeout" : function(response, stausText, xhr){
				handelBootstrapToastr({status:"error",msg:"</br>操作请求超时，最大时间：五分钟",timeOut:"10000"});
			}
		};
	};
	...
	function commonAjaxErrorCallback(response,option,isRest){
		...
		if(isRest&&option.btnName){
			handelBootstrapToastr({status:"error",msg:"</br>"+option.btnName+"失败:"+"</br></br>"+(jsonResponse.errorMsg+""),timeOut:"10000"});
		}
		//定义回调函数
...
//					$(obj).selectpicker('val',objValue);
			...
			if(objValue!=undefined){
				if($(obj).attr("type")=="checkbox"){
					...
					//更新日期范围组件不用了	 
					}else if($(obj).parent().hasClass("input-daterange_discharge")||parentRemoveClass&&parentRemoveClass.indexOf("input-daterange_discharge")!=-1){
	...
	var getDmsValue = function(item,container){
		...
		if($(item).attr("type")=="text"||$(item).attr("type")=="hidden"||$(item).is('textarea')||$(item).attr("type")=="password"){
			//如果roleTagesInput
		...
		if($(item).attr("type")=="radio"){
			var name = $(item).attr("data-inputName")!=undefined?'data-inputName="'+$(item).attr("data-inputName")+'"':'name="'+$(item).attr("name")+'"';
			var value = $(item).attr("data-isRadio")!="true"?$('input[type="radio"]['+name+']:checked',$(item).closest("div.radio-list,td.bs-checkbox")).val():$(item).val();
	...
	function getFormatValue(obj,objValue){
		if(objValue){
			...
			if($(obj).attr("data-dictCode")){
				objValue=formatDictSpanDate(obj,objValue);
			}
		}
		return objValue;
	}
	 ...
	 * 页面控制权限
	 */
	function getBusinessPurview(ctrlCode,callback){
		//进行ajax 请求
		/*dmsCommon.ajaxRestRequest({
			url:dmsCommon.getDmsPath()["manage"]+"/basedata/userCtrl/"+ctrlCode+"/getBusinessPurview",
			type:'GET',
	        sucessCallBack:function(data){
	        	ctrlCodeDataMap = data;
	        	(callback && typeof(callback) === "function") && callback(data);
	        }
		});*/
    	var type=ctrlCode.substring(0,1);
    	if(type=='1'){
    		return commonDataMap['userInfo']['receptionMaintain'][ctrlCode];
    	}
    	if(type=='2'){
    		return commonDataMap['userInfo']['partsOption'][ctrlCode];
    	}
    	if(type=='3'){
    		return commonDataMap['userInfo']['settleAccounts'][ctrlCode];
    	}
    	if(type=='5'){
    		return commonDataMap['userInfo']['discountModeOption'][0];
    	}
    	if(type=='7'){
    		return commonDataMap['userInfo']['systemAttention'][ctrlCode];
    	}
	 ...
	 * 设置锁定页面的值
	 */
	var setLockData = function(lockData){
		$("#dmsTabDiv > div.tab-content > div.tab-pane.active").data("LockOrder",lockData);
	}
	
	/**
	 * 清空锁定页面的值
	 */
	var clearLockData = function(dmsFuncContent){
		$(dmsFuncContent).data("LockOrder",undefined);
	}
	
	/**
	 * 解除锁定页面的值
	 */
	var unLockPage = function(content){
		var lockData=$("#dmsTabDiv > div.tab-content > div.tab-pane.active").data("LockOrder");
		if(!lockData) return;
		var dmsFuncContent=content||$("#dmsTabDiv > div.tab-content > div.tab-pane.active");
		$("#" + lockData.btnName,dmsFuncContent).trigger("click");
		clearLockData(dmsFuncContent);
	}
	/*
	* 读取缓存的参数
	*/
	var getParamsStorage=function(name){
	...
	return {
        ...
        getBusinessPurview:function(ctrlCode){
        	if(ctrlCode==undefined){
        		return false;
        	}
        	return getBusinessPurview(ctrlCode,function(data){
     		});
        },
        getSystemDate : function(){
        	return systemDate;
        },handelAllFile:function(container){
        	handelAllFile(container);
        },
        ...
        },setLockData : function(lockData){
        	setLockData(lockData);
        },
        clearLockData : function(){
        	clearLockData();
        },
        unLockPage : function(content){
        	unLockPage(content);
        },
        /**
         * 获得当前激活的菜单
        ...
        getDmsActiveFuncTab : function(){
        	return $("#dmsPageContent").size()>0?$('div.page-content div.page-content-body'):$("#dmsTabDiv > div.tab-content > div.tab-pane.active");
        },
...
(function($) {
	 ...
	 * 设置字段的值，包括各种情况
	 */
	$.fn.getDmsValue = function(objValue){
		var obj = this;
		return dmsCommon.getDmsValue(obj,objValue);
	};
	...
	$.fn.getParentTab = function(){
		var selector = $(this);
		return $("#dmsPageContent").size()>0?$("#dmsPageContent"):selector.closest(".tab-content .tab-pane");
	}
