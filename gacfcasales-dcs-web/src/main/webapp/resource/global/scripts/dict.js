var dmsDict = function() {
	...
	function initCheckBoxValue(valueArray,checkBoxObj,valueName,descName,checkBoxContainer){
		var isDisabled=$(checkBoxObj).attr("data-disabled");
		...
		if(displayStyle!=undefined&&displayStyle=="vertical"&&dictLabelName!=undefined){
			var isShowallSel=$(checkBoxObj).attr("data-isShowallSel")=="true" ? true : false; 
			var htmlStr = "";
			if(isShowallSel){
				htmlStr="<label class='checkbox-title' ><input type='checkbox' name='"+name+"_all' id='"+(id)+"_all' ><span>"+dictLabelName+"</span></label>";
			}else{
				 //htmlStr = "<label class='checkbox-title' ><span>"+dictLabelName+"</span></label>";
				htmlStr="<label class='checkbox-title ' ><input type='checkbox' hidden='true' disabled='disabled' name='"+name+"_all' id='"+(id)+"_all' ><span>"+dictLabelName+"</span></label>";
			}
		...
		$.each(valueArray,function(j,value){
    		...
    		//定义checkBox 信息
    		var disabledy="";
    		if(dictValue[isDisabled]!=undefined&&dictValue[isDisabled]=='10041001'){//为什么中括号不行？
    			disabledy='disabled';	
    		}
			var htmlStr = "<input type='checkbox'  name='"+name+"' id='"+(id+j)+"' "+disabledy+" "+(inputName!=undefined?"data-inputName='"+inputName+"'":"")+" value='"+dictValue[valueName]+"'>";
			if(j!=0){
				checkBoxArray.push(htmlStr);
			}else{
				$(checkBoxObjClone2).attr("id",id+j);
				$(checkBoxObjClone2).attr("value",dictValue[valueName]);
				if(!isStringNull(disabledy)){
					$(checkBoxObjClone2).prop("disabled",true);
				}
	...
	function updateCheckRadio(checkBoxObj,checkBoxContainer,obj){
		$(".checkbox-vertical>input[name^='roleMenuRange']").each(function () {
			  if (this != obj)
	...
	function bindCheckAllEvent(checkBoxObj,checkBoxContainer){
		...
		$("div.checkbox-list .checkbox-vertical input[type='checkbox']",checkBoxContainer).bindChange(function(obj){ 
			if($(checkBoxObj).attr("is_CheckRadio")=='true'){
				updateCheckRadio(checkBoxObj,checkBoxContainer,obj);
			}else{
				if(!$(obj).prop('checked')){
	...
	function initCheckBox(checkBoxObj) {
		...
		if ($(checkBoxObj).attr("data-model")) {
			...
			var excluedeItems = $(checkBoxObj).attr("data-excludeItems");
			if (!isStringNull(url)) {
				dmsCommon.ajaxRestRequest({
					...
					sucessCallBack : function(data) {
						// 初始化数据
						initCheckBoxValue(data, checkBoxObj, $(checkBoxObj).attr("data-labelValue"), $(checkBoxObj).attr("data-lableDesc"), checkBoxContainer,excluedeItems);		
					}
				});
			}
		} else {
			if (dictDataMap) {
				...
				if(type=='dict'){
					...
					// 初始化数据
					initCheckBoxValue(dictValueArray, checkBoxObj, "code_id","code_cn_desc", checkBoxContainer,excluedeItems);
							
				}else if(type=='oemDict'){
					dictData = oemDictDataMap[type];
					var dictValueArray = dictData[dictCode];
					// 初始化数据
					initCheckBoxValue(dictValueArray, checkBoxObj, "code_id","code_desc", checkBoxContainer,excluedeItems);
		...
		if (model) {
			...
			if (dictDataMap) {
				...
				if (type == 'dict') {
					...
					dictData = oemDictDataMap[type];
					var dictValueArray = dictData[dictCode];
					initRadioValue(dictValueArray, radioBoxObj, "code_id","code_desc");
	 ...
	 */
	function initSelectOneOption(selectObj, optionValue, valueName, descName,
			excluedeItems) {
		if (excluedeItems == undefined
				|| excluedeItems.indexOf(optionValue[valueName]) == -1) {
			var optionDesc;
	...
	function initSelectData(selectObj) {
		if ($(selectObj).attr("data-model")) {
			...
			if (dictDataMap) {
				var type = $(selectObj).attr("data-type"); type = (type == undefined ? "dict" : type);
				var labelValue=$(selectObj).attr("data-labelValue")||"code_id";
				...
				if (type == "dict") {
					...
					initSelectValue(dictValueArray, selectObj, labelValue,"code_cn_desc");
							
				}
				if (type == "oemDict") {
					dictData = oemDictDataMap[type];
					dictValueArray = dictData[dictCode];
					initSelectValue(dictValueArray, selectObj, "code_id","code_desc");
				...
				if (type == "oemRegion") {
					...
					initSelectValue(dictValueArray, selectObj, "region_code","region_name");
	...
	function initSelect(selectObj) {
		var parentDiv = $(selectObj).closest(dmsCommon.DMS_CLOSEST_DIV);
		// 绑定parent 事件
		$(selectObj).data("cascadeData", {});
		bindSelectParentEvent(selectObj, selectObj, parentDiv);
		// 执行select 数据初始化
	 ...
	 * 绑定parent 事件
	 */
	function bindSelectParentEvent(selectObj, rootSelect, container) {
		var selectId = $(selectObj).attr("id");
		// 绑定parent 事件
		if ($("select[parent='" + selectId + "']", container).size() > 0) {
			$(selectObj).off("change").on(	"change",function() {

				if ($(selectObj).attr("parent") == undefined) {
					 $(rootSelect).data("cascadeData", {});
				 }
					var rootCascadeData = $(rootSelect).data("cascadeData");
					     ...
					     $.each($("select[parent='" + selectId+ "']", container), function(i, childObj) {
							    ...
							    if (currentVal) {
									 var parentId = $(selectObj).attr("id");
										 rootCascadeData[parentId] = currentVal;
										// 复制datatype 属性
									    ...
									    // 执行动态查询
										selectChangeEvent(childObj,rootCascadeData);
															
									});
							});

			$.each($("select[parent='" + selectId + "']"),
					function(i, childObj) {
						// 递归绑定
						bindSelectParentEvent(childObj, rootSelect, container);
					});
		}
	}
	/**
	...
	function isInit(obj) {
		...
		if ($(obj).is('select')) {
			...
			// return ($(obj).parent("div").attr("data-isInit") ==
			// "true"&&($(obj).attr("data-alwaysRefresh")==undefined||$(obj).attr("data-alwaysRefresh")=="false"));
		} else if ($(obj).attr("type") == "radio") {
			// return
			// ($("label.radio-inline",$(obj).parent().parent()).size()>0);
			return ($(obj).attr("data-isInit") == "true");
		} else if ($(obj).attr("type") == "checkbox") {
			// return
			// ($("label.checkbox-inline",$(obj).parent().parent()).size()>0);
			return ($(obj).attr("data-isInit") == "true");
		}
	}
	/**
	...
	var init = function(container) {
		var objList = $(
				"select:not([parent]),input[type='radio']:not([data-index]),input[type='checkbox']:not([data-index],[name='btSelectAll'])",
				container);
		// 对组件进行初始化
	...
	var initData = function(obj) {
		if (isInit(obj)) {
			...
			if (objValue != undefined) {
				...
				// 触发dms.change
				if ($(obj).attr("type") == "checkbox") {
					var elementContainer = $(obj).parent().is('label') ? $(obj)
							.closest("div.checkbox-list").parent() : $(obj)
							.parent();
					// 进行触发
	...
	var getCodeDescById = function(type, codeType, codeId) {
		if (isStringNull(codeId)) {
			...
			var reduced = [];
			if(type=='dict'){
				codeList = dictDataMap[type][codeType];
				// 获取符合ID 描述
				...
				if(codeType.split(',').length>1){
					for(var i=0;i<codeType.split(',').length;i++){
						codeList = oemDictDataMap[type][codeType.split(',')[i]];
						// 获取符合ID 描述
						var codeObjectArray = $.grep(codeList, function(item, index) {
							return item.code_id == codeId;
						});
						$.each(codeObjectArray, function(j, dictValue) {
							desc = dictValue.code_desc;
						});
					}
				}else{
					codeList = oemDictDataMap[type][codeType];
					// 获取符合ID 描述
					var codeObjectArray = $.grep(codeList, function(item, index) {
						return item.code_id == codeId;
					});
				...
				codeList = dictDataMap[type];
				if(codeType=='provice'){
					var RegionObjectArray = $.map(codeList, function(item, index) {
						return index == codeId?item:null;
					});
					$.each(RegionObjectArray, function(j, item) {
						desc = item.region_name;
					});
				}else if(codeType=='city'){
					var RegionObjectArray = $.map(codeList, function(item, index) {
						return  $.map(item.children, function(Cityitem, index) {
							return index == codeId?Cityitem:null;
						});
					});
					$.each(RegionObjectArray, function(j, item) {
						desc = item.region_name;
					...
					var RegionObjectArray = $.map(codeList, function(item, index) {
						return $.map(item.children, function(Cityitem, index) {
							$.map(zitem.children, function(countryitem, index) {
								return index == codeId?countryitem:null;
							});
						});
					});
					$.each(RegionObjectArray, function(j, item) {
	...
	var getDictData = function() {
		...
		// 进行ajax 请求
		dmsCommon.ajaxRestRequest({
			url : dmsCommon.getDmsPath()["web"] + "/basedata/oemDicts",
			type : 'GET',
			sucessCallBack : function(data) {
				oemDictDataMap = data;
			}
