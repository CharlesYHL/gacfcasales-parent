...
(function($){
	...
	data = {
		...
		getNodesByParam: function(setting, nodes, key, value) {
			...
			result = [];
			for (var i = 0, l = nodes.length; i < l; i++) {
				if (nodes[i][key].substr(nodes[i][key].indexOf("_")+1).indexOf(value)>-1 && nodes[i]['level']!=1) {
					result.push(nodes[i]);
				}
	...
	view = {
		...
		makeDOMNodeIcon: function(html, setting, node) {
			var nodeId;
			if(node.level > 1){
				nodeId=node.id.substr(node.id.indexOf("_")+1);
			}else{
				nodeId = "";
			}
			...
			html.push("<span id='", node.tId, consts.id.ICON,
				...
				"' style='", view.makeNodeIcoStyle(setting, node), "'></span><font>",nodeId,"&nbsp;</font><span id='", node.tId, consts.id.SPAN,
