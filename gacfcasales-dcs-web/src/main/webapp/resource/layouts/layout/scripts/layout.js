...
var Layout = function () {
    ...
    var handleSidebarMenu = function () {
        ...
        $('.page-sidebar').on('click', ' li > a.ajaxify', function (e) {
            ...
            //清理资源
//            dmsCommon.cleanPageCache();
           
            if($("#sysType").text()==1){
            	execBindMenuClick(e,this);
            }else{
            	var url = $(the).attr("href");
                var pageContent = $('.page-content');
                var menuId = $(the).parents('li').attr("id");
                var menuName = $(the).text();
                var uili= $("#dmsTabDiv >ul li").size(); 
               if(uili>6){
            	   dmsCommon.tip({status:"warning",msg:"最多只能开6个页签！"});
            	   return;
               }
                //改变菜单的Class
                var tabContentDiv = dmsIndex.changeActiveFunc(menuId,menuName,url);
	 ...
	 * 页面初始页面
	 */
    var execBindMenuClick = function(e,obj){
		e.preventDefault();
		App.scrollTop();
		var the = $(obj);
		...
		//清理资源
		dmsCommon.cleanPageCache();
		if(!$("#dmsPageContent").size()>0){
			$('.page-content-wrapper .page-content').html('<div class="page-bar"></div>  <div id="dmsPageContent" class="page-content-body">	');
		}
		...
		dmsCommon.ajaxPageRequest({
			...
			success:function(html){
				//还原查询条件
				dmsIndex.changeFuncCacheData_Plain(menuId);
			},
    ...
    $(".nav-tabs").contextmenu({
        ...
        onItem: function (context, e) {
        	...
        	if("关闭当前"==operAction){
        		var curTabArr = $("#dmsTabDiv >ul.nav-tabs > li.active i.glyphicon-remove");
        		if(null != curTabArr && curTabArr.length>0){
        			closeTab(curTabArr[0]);
        		}
        	}else if("关闭其他"==operAction){
        		/**
        		 * 先处理下拉隐藏的，后续处理显示的
        		 */
        		var liTabDropDownArr = $("#dmsTabDiv .nav-tabs ul.dropdown-menu > li")
        		for(var i=0;i<liTabDropDownArr.length;i++){
        			var tempTab = liTabDropDownArr[i];
        			if(!$(tempTab).hasClass("active")){
        				closeTab($(tempTab).children(":first").children(":first")[0]);
        			}
        		}
        		
        		var liTabArr = $("#dmsTabDiv >ul.nav-tabs > li");
        		for(var i=1;i<liTabArr.length;i++){
        			var tempTab = liTabArr[i];
        			if(!$(tempTab).hasClass("active")){
        				closeTab($(tempTab).children(":first").children(":first")[0]);
        			}
        		}
