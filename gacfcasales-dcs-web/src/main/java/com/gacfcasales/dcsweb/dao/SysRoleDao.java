...
public interface SysRoleDao{
	...
	Long selectSysRoleByUserId(@Param("userId") Long userId);
	
	public int insertResources(Map map);
	
	public int deleteResources(Long id);
	
	List selectResourceByRoleId(Long roleId);
	
	
	
	Long selectSysRoleByUserIdAndCode(Map map);
	
	Map findByAppIdAndUsername(@Param("appId")String appId, @Param("username")String username);
}